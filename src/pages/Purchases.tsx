...
import { Plus, Trash2, Search, ShoppingCart, Calendar } from 'lucide-react';
...
export default function Purchases() {
  ...
  const todayStats = useMemo(() => {
    const todayPurchases = purchases.filter(p => p.date === formatDate(new Date()));
    return {
      total: todayPurchases.reduce((sum, p) => sum + p.totalPrice, 0),
    ...
    <div className="space-y-4 pb-20 lg:pb-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">{purchases.length} عملية شراء</p>
        </div>
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full" disabled={products.length === 0}>
                {products.length === 0 ? 'أضف منتجات أولاً' : 'تسجيل الشراء'}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-primary/10 border-primary/20">
          <CardContent className="p-3 text-center">
            <p className="text-xs text-muted-foreground">مشتريات اليوم</p>
            <p className="text-lg font-bold text-primary">{formatCurrency(todayStats.total)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3 text-center">
